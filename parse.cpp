 ...
 */
	count_elts = 0;
	strcpy_s(token, trxn.token[0].name);
	replace("(s)", "", token);
	replace("(S)", "", token);
	replace("(g)", "", token);
	replace("(G)", "", token);
    ...
    if (get_elts_in_species(&char_ptr, trxn.token[0].coef) == FALSE)
	{
        return (FALSE);
	}
/*
 ...
 */
	...
	for (i = 0; i < count_trxn; i++)
	{
        ...
        if (get_elts_in_species(&t_ptr, trxn.token[i].coef) == FALSE)
		{
 ...
 */
            if (get_elt(t_ptr, element, &l) == FALSE)
			{
                return (FALSE);
			}
 ...
 */
		if (c == '(')
		{
			...
			paren_count++;
			(*t_ptr)++;
            if (get_elts_in_species(t_ptr, coef) == FALSE)
			{
                return (FALSE);
			}
            if (get_num(t_ptr, &d) == FALSE)
 ...
 *   Get new element and subscript
 */
            if (get_secondary(t_ptr, element, &l) == FALSE)
			{
                return (FALSE);
			}
			elt_list[count_elts].elt = element_store(element);
 ...
 */
		if (c == '(')
		{
			...
			paren_count++;
			(*t_ptr)++;
            if (get_secondary_in_species(t_ptr, coef) == FALSE)
			{
                return (FALSE);
			}
            if (get_num(t_ptr, &d) == FALSE)
 ...
 */
	...
	/* coefficient */
    if (get_coef(&(trxn.token[count_trxn].coef), ptr) == FALSE)
	{
        return (FALSE);
	}
	/* name and charge */
    if (get_token(ptr, string, &trxn.token[count_trxn].z, &l) == FALSE)
	{
        return (FALSE);
	}
