 ...
 *
 * With count_stag = 1, mix factors are calculated from exchange factor ? * (= exch_f), mobile é_m (= th_m) and immobile é_im (= th_im) porosity.
 ...
 */
		if (b_c == 1)
		{
			...
			for (j = 1; j <= floor((LDBLE) nmix / 2); j++)
			{
				...
				if (transp_surf)
				{
                    if (disp_surf(stagkin_time) == FALSE)
