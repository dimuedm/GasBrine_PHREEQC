 ...
 */
		...
		for (int j = 0; j < count_elts; j++)
		{
				 ...
				 rxn_ptr->s != NULL; rxn_ptr++)
			{
				...
				{
                    if (ReSoCMode_DEBUG) // Li Jun added 2018-5-1
                    {
                        error_string = sformatf(
 ...
 */
		...
		if (comp_ref.Get_gfw() <= 0.0)
		{
			if (comp_ref.Get_as().size() > 0)
			{
                ...
                if (compute_gfw(comp_ref.Get_as().c_str(), &dummy) == FALSE)
				{
 ...
 */
	for (i = 0; i < count_unknowns; i++)
	{
		...
		if (x[i]->type == PP)
		{
			...
			cxxPPassemblageComp * comp_ptr = (cxxPPassemblageComp * ) x[i]->pp_assemblage_comp_ptr;
			si_org = comp_ptr->Get_si_org();
			if (phase_ptr->p_c > 0 && phase_ptr->t_c > 0)         //This part is deleted by Jun Li, 2018-4-8; gas fugacity is calculated outside.
			{
				/*if (si_org > 3.5)
					si_org = 3.5;
				p = exp(si_org * LOG_10);
 ...
 */
	...
	for (i = 1; i < count_trxn; i++)
	{
		...
		else
		{
			char * temp_name = string_duplicate(trxn.token[i].s->secondary->elt->name);
			char * ptr = temp_name;
            if (get_secondary_in_species(&ptr, trxn.token[i].coef) == FALSE)
			{
				input_error++;
 ...
 */
	for (i = 0; i < count_phases; i++)
	{
		if (phases[i]->in == TRUE)  
		{
            ...
            /*std::cout << phases[i]->name << "  " << phases[i]->lk << std::endl;*/
		}
	}
/*
