...
/* ---------------------------------------------------------------------- */
{
	...
	for (i = 0; i < logk_ptr->count_add_logk; i++)
	{
		...
		if (next_logk_ptr->done == FALSE)
		{
            ...
            if (add_logks(next_logk_ptr, repeats + 1) == FALSE)
			{
                return (FALSE);
			}
		}
 ...
 */
	...
	for (i = 0; i < count_master; i++)
	{
		...
		{
            if (compute_gfw(master[i]->gfw_formula, &master[i]->gfw) == FALSE)
			{
				input_error++;
				error_string = sformatf(
...
/*   Calculation of l_delta[n] */
	...
	{
	if(ReSoCMode_DEBUG)	output_msg(sformatf( "Error: Divide by zero in slnq.\n"));
		l_delta[n] = 0.0;
		goto slnq_error;
	}
