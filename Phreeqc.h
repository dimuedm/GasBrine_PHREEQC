...
class cxxReaction;
class cxxSolution;
class cxxISolution;   //Li Jun added - 2017-12-19
...
// Li Jun changed: first two arguments were deleted!
	int process_file_names(std::istream **db_cookie,
		std::istream **input_cookie, 
        std::string *inputFileName, std::string *outFileName, std::string *databaseFileName,
        int log);
    ...
    // for ReSoC use ------------------------------
    int run_simulation_timeStep(conditionChange &);
    bool initialReSoC = false;
    void set_ReSoC_intitial(bool initial);
    int reactions_ReSoC(float deltaTime);
    void use_update_solution(conditionChange &);
    void update_solution_totals(conditionChange &);
    void updateLogK(std::string, double logK);
    void print_solution(int n_user);
 //   int update_solution(void);
    void master_species_change_solution(void);
    void gas_composition_update(void);
    void use_pp_update(std::map<std::string, double> &mineralUpdate);
    void use_gas_update(conditionChange &);
    void use_reaction_update(conditionChange &);
    void newTimeStep_reset(void);
    void results_for_ReSoC(gasPhase_results &gas, waterSolution_results &water, std::map<std::string, minerals> &mineral, std::map<std::string, kineticPhases> &kineticMinerals);
    void results_for_ReSoC(gasPhase_results &gas, waterSolution_results &water, std::map<std::string, minerals> &mineral);
    LDBLE calc_dDens_dp(LDBLE);
    LDBLE calc_solution_volume_ReSoC(LDBLE density_solution);
    LDBLE calc_viscosity();
    LDBLE calc_dVis_dp();
    bool ReSoCMode;
    bool ReSoCMode_DEBUG;
    void update_ReSoC_timeStep(LDBLE temperature, LDBLE pressure);
    void ReSoC_screen_print(gasPhase_results &gas, waterSolution_results &water );
    bool gasSolubilityCalculation;
    bool ReSoC_timeStepCalc;
    int initial_solutions_ReSoC(int n_user);
    int initial_solutions_ReSoC(int n_user, LDBLE temperature, LDBLE pressure);
    int calActivityCoeff(conditionChange &solutionCondition, map<string, speciesComp> &activityCoeff);
    int xpp_assemblage_save_mineralOnly(int n_user);
    void setup_initialSolution_ReSoC(cxxSolution *, double massOfH2O);
    void creatIniSolutionData_ReSoC(cxxISolution* ISolution_ptr, cxxSolution *solution);
    void tidy_pp_assemblage_ReSoCUpdate(cxxPPassemblage* pp_assemblage_ptr);
//    void save_pp_rxn_map(int);
    int recalc = 0;
    std::map<int, std::string> masterSpecies_aq;
    std::map<int, std::string> *get_masterSpecies_aq();
    std::map<std::string, double> masterMolality_aq;
    std::map<std::string, double> *get_masterMolality_aq();

    GasComponentDataBase gasComponentDatabase;

  /*  void setup_newGasPhase(conditionChange &update_timestep);*/
...
public:
	...
	// mainsubs.cpp  -------------------------------
	std::ifstream * open_input_stream(std::string *default_name, std::ios_base::openmode mode, bool batch); //The first argument is deleted. The second argument type is changed to string from char. Li Jun 2017-8-3
    std::ofstream * open_output_stream(std::string *default_name, std::ios_base::openmode mode, bool batch); //The first argument is deleted. The second argument type is changed to string from char. Li Jun 2017-8-3
...
public:
//	int main_method(int argc, char *argv[]);  //Li Jun comments it! 2017-8-2
	void set_phast(int);
...
public:
    ...
    //The following is that Li Jun added for testing 2017-8-2
public:
    void printMaster();
